...
var websocketURLParam = searchParams.get("server");
...
var realtimeActionHistory = [];

var rectanglePlaced = true;
var outlineRectPlaced = true;
var linePlaced = true;
...
setInterval(() => {
  try {
    ...
    ctx.fillStyle = color.value;
    var lastaction = realtimeActionHistory[realtimeActionHistory.length - 1];
    var colorActionNeeded = true;
    if (lastaction) {
      if (lastaction[0] == "color") {
        if (lastaction[1] == color.value) {
          colorActionNeeded = false;
        }
      }
    }
      ...
      if (down) {
        try {
          for (let i = 0; i < 3; i++) {
            ...
            ctx.fillRect(values[1], values[2], values[3], values[4]);
            realtimeActionHistory.push(values);
          }
        } catch (e) {
          window.alert(e);
        }
...
if (websocketURLParam) {
  var socket = new WebSocket(websocketURLParam);
  ...
  function doActionHistory(actHistory) {
    for (var part of actHistory) {
      if (part[0] == "color") {
        ctx.strokeStyle = part[1];
        ctx.fillStyle = part[1];
        ctx.lineWidth = part[2];
      }
      if (part[0] == "fillRect") {
        ctx.fillRect(part[1], part[2], part[3], part[4]);
      }
      if (part[0] == "clearRect") {
